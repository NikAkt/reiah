...
const Filter = ({ realEstateData, amenitiesData }) => {
  ...
  for (let key of Object.keys(amenitiesData)) {
    ...
    obj.forEach((arr) => {
      ...
      if (!amenities_type_desc[arr["FACILITY_T"]].includes(arr["FACILITY_DOMAIN_NAME"])) {
        amenities_type_desc[arr["FACILITY_T"]].push(arr["FACILITY_DOMAIN_NAME"]);
      }
    });
  }
  ...
  return (
    <div class="absolute z-30 w-[80vw] flex flex-col items-center gap-0.5 top-[2vh] left-[10vw] justify-center text-black">
      <button
        class="rounded shadow-md color-zinc-900 cursor-pointer bg-white text-base mt-4 mx-6 mb-6 leading-9 py-0 px-2 text-center"
        onClick={toggleFilter}
      >
        <span>Filter</span>
      </button>
      {filterDisplay() && (
        <div class="grid-cols-1 divide-y m-0 px-0 mt-[-2vh] w-full h-[80vh] shadow-md z-20 items-center bg-white animate-fade-down rounded-lg transition-all duration-500 overflow-y-auto relative">
          {/* FILTER TITLE */}
          <div id="filter-dropdown-title" class="items-center justify-center relative flex h-[8%] bg-black text-white w-[100%] z-30 flex-row rounded-t-lg" style="position: sticky; top: 0;">
            <button
          ...
          <div class="w-[100%] flex flex-col h-auto relative items-center py-4 px-[10%] gap-y-2.5 bg-white" id="filter-details-container">
            ...
            {[...selectedBoroughs()].length > 0 && (
              <div id="neighborhood-container" class="border-solid border-2 border-indigo-600 w-full p-4 rounded-lg transition-all duration-500 ease-in-out transform opacity-100 scale-100">
                ...
                <div class="w-full h-32 overflow-y-auto border border-gray-300 rounded-md">
                  {getNeighborhoods([...selectedBoroughs()]).map((el) => (
                    <div key={el} class={`p-2 cursor-pointer ${selectedNeighborhoods().has(el) ? 'bg-indigo-600 text-white' : 'bg-white text-black'}`} onClick={() => handleNeighborhoodChange(el)}>
                      {el}
                    </div>
                  ))}
                </div>
            ...
            {[...selectedNeighborhoods()].length > 0 && (
              <div id="zipcode-selection-container" class="border-solid border-2 border-indigo-600 w-full p-4 rounded-lg transition-all duration-500 ease-in-out transform opacity-100 scale-100">
                <label htmlFor="zipcode-selection" class="font-sans text-2xl font-bold text-black">ZIPCODE:</label>
            ...
            {selectedZipCodes().size > 0 && (
              <button class="mt-4 p-2 bg-indigo-600 text-white rounded transition-all duration-500 ease-in-out transform opacity-100 scale-100" onClick={() => setShowAdvancedFilters(!showAdvancedFilters())}>
            ...
            {/* Advanced Filters */}
            {showAdvancedFilters() && (
              <div id="advanced-filters-container" class="w-full p-4 border-solid border-2 border-indigo-600 rounded-lg mt-4 flex flex-col transition-all duration-500 ease-in-out transform opacity-100 scale-100">
          ...
          <div id="button-container" class="items-center justify-center flex gap-4 h-[8%] relative bg-black text-white w-[100%] z-30 border-solid border-2 border-indigo-600 rounded-b-lg p-4" style="position: sticky; bottom: 0;">
            <button
